...
class ProxyGenerator implements Generator {
  ...
  String _positionalArgsList(MethodElement method) => method.formalParameters.where((p) => !p.isNamed).map((p) => p.name).join(', ');
  ...
  String _namedArgsMap(MethodElement method) => method.formalParameters.where((p) => p.isNamed).map((p) => "'${p.name}': ${p.name}").join(', ');
